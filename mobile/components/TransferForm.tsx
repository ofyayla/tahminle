  ...
  return (
    <View style={{ gap: 20 }}>
      <View>
        ...
        {targets.length === 0 ? (
          <Text style={styles.empty}>Bakiye gönderebileceğin bir taraftar yok.</Text>
        ) : (
      ...
      {confirming ? (
        <View style={styles.confirmBox}>
          <Text style={styles.confirmText}>
            ...
            <Text style={{ fontFamily: fonts.bold }}>{recipient?.displayName}</Text> adlı oyuncuya göndereceksin.
          ...
          <View style={styles.confirmRow}>
            ...
            <Pressable style={[styles.primaryBtn, { flex: 1 }, busy && { opacity: 0.5 }]} disabled={busy} onPress={submit}>
              {busy ? <ActivityIndicator color={colors.bg} /> : <Text style={styles.primaryText}>Onayla ve Gönder</Text>}
            </Pressable>
          </View>
        </View>
      ) : (
          ...
          <View style={{ gap: 8 }}>
            {history.map((h) => (
              <View key={h.id} style={styles.historyRow}>
                ...
                <Text style={[styles.historyAmount, { color: h.direction === "out" ? colors.red : colors.green }]}>
                  {h.direction === "out" ? "−" : "+"}
                  {formatTL(h.amount)}
                </Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  historyWrap: { borderTopWidth: 1, borderTopColor: colors.cardBorder, paddingTop: 16 },
