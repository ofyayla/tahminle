  ...
  return (
    <section className="rounded-2xl border border-card-border bg-card p-4">
      <button type="button" onClick={() => setOpen((o) => !o)} className="flex w-full items-center gap-3 text-left">
        <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-green/15 text-green">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4">
            <path d="M4 12h13M13 6l6 6-6 6" />
          </svg>
        </div>
        <div className="min-w-0 flex-1">
          <div className="font-display text-base">Bakiye Gönder</div>
          <div className="text-xs text-ink-dim">Başka bir taraftara sanal bakiye aktar</div>
        </div>
        ...
        <div className="mt-4 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {targets.map((t) => (
                <button
                  key={t.id}
                  type="button"
                  onClick={() => { setRecipientId(t.id === recipientId ? "" : t.id); setConfirming(false); }}
          ...
          <div>
            ...
            <input
              ...
              max={available}
              value={amount}
              onChange={(e) => { setAmount(Math.floor(Number(e.target.value) || 0)); setConfirming(false); }}
          ...
          {confirming ? (
            <div className="rounded-xl border border-gold/40 bg-gold/5 p-3">
              <p className="text-sm text-ink">
                ...
                <span className="font-bold">{recipient?.displayName}</span> adlı oyuncuya göndereceksin.
            ...
            <button
              type="button"
              disabled={!canSubmit}
              onClick={() => { setDone(null); setError(null); setConfirming(true); }}
