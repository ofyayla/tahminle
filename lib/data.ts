...
// Ranks players by virtual balance — the leaderboard is a straight reflection
// of the wallet, so a win moves you up the board the moment it settles.
export async function getLeaderboard(currentUserId: string) {
  const users = await prisma.user.findMany({
    orderBy: [{ balance: "desc" }, { displayName: "asc" }],
    select: { id: true, displayName: true, favoriteTeam: true, balance: true },
  });

  const ranked: LeaderboardRow[] = users.map((u, i) => ({
    rank: i + 1,
    id: u.id,
