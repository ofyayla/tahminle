...
const HERE = path.dirname(new URL(import.meta.url).pathname); // frames*.json live next to this script
const CHROME = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";
const FONTS = path.join(ROOT, "mobile/node_modules/@expo-google-fonts");

// One canvas per store slot. The composition is the same everywhere; what
// changes is the canvas, the two numbers that reflow it (where the copy starts,
// how big the device is), and which raw screenshots feed it.
//   play     — Play Console phone screenshots: 9:16, 320-3840px per side.
//   appStore — App Store Connect 6.7" iPhone slot (1284x2778).
//   ipad     — App Store Connect 12.9"/13" iPad slot (2064x2752).
// `type` overrides the default type scale (canvas width / 1080); the iPad
// canvas is wide enough that a straight scale would shout.
const IPHONE_RAW = { dir: "raw", shotW: 1206, shotH: 2622, ptWide: 402 }; // iPhone 17 Pro @3x
const IPAD_RAW = { dir: "raw-ipad", shotW: 2064, shotH: 2752, ptWide: 1032 }; // iPad Pro 13" @2x

const PROFILES = {
  play: {
    dir: ".", w: 1080, h: 1920, copyTop: 112, phoneTop: 480, screenW: 636,
    frames: "frames.json", raw: IPHONE_RAW, device: "iphone",
  },
  appStore: {
    dir: "app-store", w: 1284, h: 2778, copyTop: 140, phoneTop: 610, screenW: 950,
    frames: "frames.json", raw: IPHONE_RAW, device: "iphone",
  },
  ipad: {
    dir: "ipad", w: 2064, h: 2752, copyTop: 130, phoneTop: 700, screenW: 1450,
    frames: "frames-ipad.json", raw: IPAD_RAW, device: "ipad", type: 1.75,
  },
};
...
function metrics(p) {
  const type = p.type ?? p.w / 1080; // type/padding scale
  const rail = p.screenW / (p.device === "ipad" ? 1450 : 636); // device hardware scale
  const pt = p.screenW / p.raw.ptWide; // app points -> page px, for overlays/masks
  const screenH = Math.round((p.screenW * p.raw.shotH) / p.raw.shotW);
...
// Paints over a strip of the raw screenshot (e.g. a destructive button we don't
// want in a store listing) with the app's own background colour.
function mask(k, m) {
  const px = (v) => (v * m.pt).toFixed(1);
...
// Device shell: iPhone gets a titanium rail + dynamic island, iPad gets the
// flat uniform bezel and a camera dot on the (portrait) right edge.
function shell(p, m) {
  const r = (v) => (v * m.rail).toFixed(1);
  if (p.device === "ipad") {
    return {
      pad: r(26), inner: r(6), radius: r(96), radiusInner: r(74), radiusScreen: r(68),
      rail: "linear-gradient(150deg, #4a4d55 0%, #2c2f36 22%, #6c7078 46%, #24272d 62%, #55595f 82%, #2a2d33 100%)",
      extra: "",
    };
  }
  return {
    pad: r(13), inner: r(5), radius: r(92), radiusInner: r(84), radiusScreen: r(80),
    rail: "linear-gradient(150deg, #cfd4dc 0%, #7d8492 18%, #e8ebf0 34%, #6f7683 58%, #c3c9d3 78%, #767d8b 100%)",
    extra: `<div class="island" style="top:${r(22)}px;width:${r(188)}px;height:${r(52)}px"></div>`,
  };
}

function html(f, p, rawDir) {
  const m = metrics(p);
  const s = shell(p, m);
  const t = (v) => (v * m.type).toFixed(1);
  const r = (v) => (v * m.rail).toFixed(1);
  const frameW = p.screenW + 2 * (parseFloat(s.pad) + parseFloat(s.inner));
  const frameH = m.screenH + 2 * (parseFloat(s.pad) + parseFloat(s.inner));
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .phone {
    ...
    width: ${frameW.toFixed(1)}px; height: ${frameH.toFixed(1)}px;
    border-radius: ${s.radius}px; padding: ${s.pad}px; background: ${s.rail};
    box-shadow: 0 ${r(60)}px ${r(120)}px rgba(0,0,0,.65), 0 0 0 1px rgba(255,255,255,.06);
  }
  .phone .inner { width: 100%; height: 100%; border-radius: ${s.radiusInner}px; padding: ${s.inner}px; background: #05070a; }
  .screen { position: relative; width: 100%; height: 100%; border-radius: ${s.radiusScreen}px; overflow: hidden; background: #0a0d16; }
  ...
  .glare { position: absolute; inset: 0; border-radius: ${s.radiusScreen}px; pointer-events: none;
...
<div class="stage">
  ...
  <div class="phone">${p.device === "ipad" ? s.extra : ""}<div class="inner"><div class="screen">
    <img src="data:image/png;base64,${b64(path.join(OUT, rawDir, f.shot))}">
    ${(f.masks ?? []).map((k) => mask(k, m)).join("\n    ")}
    ${(f.overlays ?? []).map((o) => overlay(o, m)).join("\n    ")}
    ${p.device === "ipad" ? "" : s.extra}
    <div class="glare"></div>
  </div></div></div>
</div>`;
}

const only = process.argv[2]; // optional: play | appStore | ipad
for (const [key, p] of Object.entries(PROFILES)) {
  if (only && only !== key) continue;
  const frames = JSON.parse(readFileSync(path.join(HERE, p.frames), "utf8"));
  const dir = path.join(OUT, p.dir);
  mkdirSync(dir, { recursive: true });
  for (const [i, f] of frames.entries()) {
    if (!existsSync(path.join(OUT, p.raw.dir, f.shot))) { console.log("SKIP (no raw):", f.shot); continue; }
    const page = path.join(os.tmpdir(), `tahminle-${key}-${i}.html`);
    writeFileSync(page, html(f, p, p.raw.dir));
